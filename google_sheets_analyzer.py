...
class ReportGenerator:
    ...
    def generate_html_report(
        ...
        html_parts = [
            ...
            ".filter-section { background: #f9fafb; padding: 25px; border-radius: 8px; margin-bottom: 30px; border: 1px solid #e8eef5; }",
            ".filter-group { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; }",
            ".filter-item { display: flex; flex-direction: column; }",
            ".filter-item label { font-weight: 600; color: #333; margin-bottom: 8px; font-size: 0.9em; }",
            ".filter-item input, .filter-item select { padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 0.95em; }",
            ...
            ".btn { padding: 10px 20px; border: none; border-radius: 6px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; font-size: 0.95em; }",
            ".btn-primary { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; }",
            ".btn-primary:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(102, 126, 234, 0.3); }",
            ".btn-secondary { background: #e8eef5; color: #667eea; }",
            ".btn-secondary:hover { background: #d4ddf5; }",
            ".filter-results { color: #666; font-size: 0.9em; margin-top: 10px; }",
            "@media (max-width: 768px) { .metrics-grid { grid-template-columns: repeat(2, 1fr); } .header h1 { font-size: 1.8em; } .filter-group { grid-template-columns: 1fr; } }",
            "</style>",
            "<script>",
            "function filterResults() {",
            "  const searchTerm = document.getElementById('searchInput').value.toLowerCase();",
            "  const minCoverage = parseFloat(document.getElementById('minCoverage').value) || 0;",
            "  const maxCoverage = parseFloat(document.getElementById('maxCoverage').value) || 100;",
            ...
            "    if (coverage < minCoverage || coverage > maxCoverage) show = false;",
            "    if (gapFilter === 'gaps' && coverage === 100) show = false;",
            "    if (gapFilter === 'full' && coverage !== 100) show = false;",
            "    ",
            "    item.style.display = show ? 'block' : 'none';",
            "    if (show && coverage < 100) visibleGaps++;",
            "  });",
            "  ",
            "  const fullItems = document.querySelectorAll('.properties-table tbody tr');",
            "  fullItems.forEach(item => {",
            "    const name = item.textContent.toLowerCase();",
            "    let show = true;",
            "    if (searchTerm && !name.includes(searchTerm)) show = false;",
            "    if (gapFilter === 'gaps') show = false;",
            "    ",
            "    item.style.display = show ? 'table-row' : 'none';",
            "    if (show) visibleFull++;",
            "  });",
            "  ",
            "  updateFilterResults(visibleGaps, visibleFull);",
            "}",
            "function resetFilters() {",
            "  document.getElementById('searchInput').value = '';",
            "  document.getElementById('minCoverage').value = '';",
            "  document.getElementById('maxCoverage').value = '';",
            "  document.getElementById('gapFilter').value = 'all';",
            "  filterResults();",
            "}",
            "function updateFilterResults(gaps, full) {",
            "  const total = gaps + full;",
            "  const resultDiv = document.getElementById('filterResults');",
            "  if (resultDiv) {",
            "    resultDiv.textContent = `Showing ${total} properties (${gaps} with gaps, ${full} with full coverage)`;",
            "  }",
            "}",
            "</script>",
            "</head>",
        ...
        html_parts.extend([
            ...
            "<h3 style='margin-bottom: 20px; color: #333;'>🔍 Filter Properties</h3>",
            "<div class='filter-group'>",
            "<div class='filter-item'>",
            "<label for='searchInput'>Search by Property Name</label>",
            "<input type='text' id='searchInput' placeholder='e.g., Badenerstr, Albis...' onkeyup='filterResults()'>",
            "</div>",
            "<div class='filter-item'>",
            "<label for='minCoverage'>Minimum Coverage (%)</label>",
            "<input type='number' id='minCoverage' min='0' max='100' placeholder='0' onchange='filterResults()'>",
            "</div>",
            "<div class='filter-item'>",
            "<label for='maxCoverage'>Maximum Coverage (%)</label>",
            "<input type='number' id='maxCoverage' min='0' max='100' placeholder='100' onchange='filterResults()'>",
            "</div>",
            "<div class='filter-item'>",
            "<label for='gapFilter'>Filter by Status</label>",
            "<select id='gapFilter' onchange='filterResults()'>",
            "<option value='all'>All Properties</option>",
            "<option value='gaps'>Only Properties with Gaps</option>",
            "<option value='full'>Only Full Coverage</option>",
            "</select>",
            "</div>",
            "</div>",
            ...
            "</div>",
            "<div class='filter-results' id='filterResults' style='color: #667eea; font-weight: 600; margin-top: 15px;'></div>",
            "</div>",
        ])
        ...
        if full_coverage_analyses:
            html_parts.extend([
                ...
                "<thead><tr><th>Property</th><th style='text-align: center;'>Coverage</th></tr></thead>",
                "<tbody>",
            ])
